@php
    $title = $resource->id ? __('Edit') . ' ' . __('Blog') : __('Add') . ' ' . __('Blog');
@endphp
...
@section('content')
    @component('dashboard.layouts.includes.card', ['title' => $title])
        @slot('content')
            <form class="form" action="{{ $resource->id ? route('admin.blog.update', $resource->id) : route('admin.blog.store') }}"
                    ...
                    <div class="col-md-6">
                        ...
                        @foreach (config('translatable.locales') as $key => $locale)
                            <div class="col-md-12 pt-3">
                                <div class="form-group">
                                    <label for="name">
                                        {{ __('Short Description'. ' ' . $locale ) }}
                                    </label>
                                    {!! Form::textarea(
                                        "{$locale}[short_description]",
                                        old("{$locale}[short_description]", optional($resource->translate($locale))->short_description),
