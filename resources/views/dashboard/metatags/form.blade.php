                ...
                <div class="row">
                    ...
                    @foreach (config('translatable.locales') as $key => $locale)
                        <div class="col-md-6 pt-3">
                            <div class="form-group">
                                <label for="name">
                                    {{ __('Page Title'.' '.$locale) }} <span class="text-danger"> * </span>
                                </label>
                    ...
                    @foreach (config('translatable.locales') as $key => $locale)
                        <div class="col-md-6 pt-3">
                            <div class="form-group">
                                <label for="name">
                                    {{ __('Description' .' '.$locale) }} <span class="text-danger"> * </span>
                                </label>
                                {!! Form::textarea("{$locale}[description]", old("{$locale}[description]", optional($data->translate($locale))->description), ['class' => 'form-control ']) !!}
                            </div>
                        </div>
                    @endforeach
                    ...
                    @foreach (config('translatable.locales') as $key => $locale)
                        <div class="col-md-6 pt-3">
                            <div class="form-group">
                                <label for="name">
                                    {{ __('keywords'.$locale) }} <span class="text-danger"> * </span>
                                </label>
                                {!! Form::text("{$locale}[keywords]", old("{$locale}[keywords]", optional($data->translate($locale))->keywords), ['class' => 'form-control']) !!}
                            </div>
                        </div>
                    @endforeach
